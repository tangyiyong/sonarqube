...
public class GlobalActionTest {
  ...
  private TestOrganizationFeature organizationFeature = TestOrganizationFeature.standalone();
  ...
  @Test
  public void organization_feature_is_enabled() throws Exception {
    init();
    organizationFeature.setEnabled(true);

    executeAndVerify("organization_feature.json");
  }
  ...
  private void init(org.sonar.api.web.page.Page[] pages, ResourceTypeTree[] resourceTypeTrees) {
    ...
    ws = new WsActionTester(new GlobalAction(pageRepository, settings, new ResourceTypes(resourceTypeTrees), server, dbClient, organizationFeature));
  }
